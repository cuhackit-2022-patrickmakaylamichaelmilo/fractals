...
proc toColorRGBX(c: ColorRGB): ColorRGBX {.inline.} =
    return ColorRGBX(
        r: c.r,
        g: c.g,
        b: c.b,
...
proc inCircle(cx: float, cy: float, r: float, x: float, y: float): bool =
    return sqrt(pow(cx - x, 2) + pow(cy - y, 2)) < r

proc drop(image: Image, xf: float, yf: float, c: ColorRGB) {.inline.} =
    let
        x = xf.int
        y = yf.int

    for x2o in -DROP_RADIUS .. DROP_RADIUS:
        for y2o in -DROP_RADIUS .. DROP_RADIUS:
            let
                x2 = x + x2o
                y2 = y + y2o

            if inCircle(xf, yf, DROP_RADIUS.float, x2.float, y2.float):
                var cN = c.toColorRGBX
                for _ in 0 .. sqrt(pow(x2o.abs.float, 2) + pow(y2o.abs.float, 2)).int div 2:
                    cN = mix(cN, image[x2, y2])
                image[x2, y2] = cN
